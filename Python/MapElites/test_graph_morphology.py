...
def vertices_to_segment_list(vertices):
    """
    Convert a list of vertices to a list of segments.
...
def create_graph(floormap, return_dist=False, room_coordinates=False):
    ...
    for i in range(int(dist.max()), int(dist.min())-1,-1):
       local_max = peak_local_max(dist, threshold_abs=threshold-i, labels=floormap, min_distance=3)
       markers = ndi.label(local_max)[0]
       if len(markers) > 0 and markers.max() > number_of_centers:
          optimal_threshold = threshold-i
          number_of_centers = markers.max()

    # Computing roommap with the optimal threshold
    local_max = peak_local_max(dist, min_distance=3, labels=floormap, threshold_abs=optimal_threshold)
    mask = np.zeros(dist.shape, dtype=bool)
    mask[tuple(local_max.T)] = True
    markers, _ = ndi.label(mask)
    roommap = watershed(-dist, markers, mask=floormap)
    roommap_color = cm.nipy_spectral(roommap)[:,:,:3]

    room_RAG_boundaries = skg.rag_boundary(roommap, filters.sobel(color.rgb2gray(roommap_color)))
    if room_coordinates:
        # For each floor...
        ...
        for floor_id in range(max(1, floors.min()), floors.max() + 1):  # Skipping label 0 (background)
            ...
            for room_id in range(max(1, rooms.min()), rooms.max() + 1):  # Skipping label 0 (background)
                ...
                room_RAG_boundaries._node[room_id]['walls'] = list()
                for segment in room_boundaries:
                    ...
                    room_RAG_boundaries._node[room_id]['walls'].append((segment, leads_to))
            ...
            for room_id in range(max(1, rooms.min()), rooms.max() + 1):
                if room_id not in rooms:
                    # Some room id may be in another floor, if they are enumerated horizontally
                    continue
                boundaries_current = {wall for wall in room_RAG_boundaries._node[room_id]['walls'] if wall[1] is None}
                for neigh in room_RAG_boundaries.adj[room_id]:
                    ...
                    boundaries_neigh = {wall for wall in room_RAG_boundaries._node[neigh]['walls'] if wall[1] is None}
                    boundaries_neigh_reverse = {_reverse_wall(wall) for wall in room_RAG_boundaries._node[neigh]['walls'] if wall[1] is None}
                    ...
                    for cs in common_segments:
                        i_current = room_RAG_boundaries._node[room_id]['walls'].index(cs)
                        i_neighbour = room_RAG_boundaries._node[neigh]['walls'].index(cs)
                        room_RAG_boundaries._node[room_id]['walls'][i_current] = (cs[0], neigh)
                        room_RAG_boundaries._node[neigh]['walls'][i_neighbour] = (cs[0], room_id)
                    # Same thing in the case of reversed segments
                    for cs in common_segments_reversed:
                        rev_cs = _reverse_wall(cs)
                        i_current = room_RAG_boundaries._node[room_id]['walls'].index(cs)
                        i_neighbour = room_RAG_boundaries._node[neigh]['walls'].index(rev_cs)
                        room_RAG_boundaries._node[room_id]['walls'][i_current] = (cs[0], neigh)
                        room_RAG_boundaries._node[neigh]['walls'][i_neighbour] = (rev_cs[0], room_id)
...
if __name__ == "__main__":
    ...
    map_matrix = [[255 if x == 1 else 0 for x in row] for row in map_matrix]

    map_matrix = np.array(map_matrix)

    roommap, room_graph, dist = create_graph(map_matrix, return_dist=True, room_coordinates=False)
    room_props = regionprops(roommap)
    for r in range(1, roommap.max() + 1):
        # Room Size
        room_graph._node[r]["area"] = room_props[r - 1]["area"]
        room_graph._node[r]["perimeter"] = room_props[r - 1]["perimeter"]
        mask = (roommap == r)
        max_dist = np.max(mask * dist)
        room_graph._node[r]["max_dist"] = max_dist
        room_graph._node[r]["centroid"] = room_props[r - 1]["centroid"]
    ...
    room_graph = ig.Graph.from_networkx(room_graph)
    room_graph.delete_vertices(0)
    room_graph.vs['centroid'] = [v['centroid'] for v in room_graph.vs]
    fig, ax = plt.subplots()
