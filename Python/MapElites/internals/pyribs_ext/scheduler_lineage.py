...
class SchedulerLineage:
    ...
    def __init__(self,
        ...
        self._lineage_table = [[] for _ in range(archive._cells)]
    ...
    def ask(self):
        ...
        for i, emitter in enumerate(self._emitters):
            ...
            self._cur_solutions.append(emitter_sols)
            if getattr(emitter, "get_last_asked_indexes", None) is not None:
                emitter_parents = emitter.get_last_asked_indexes()
                if emitter_parents is not None:
                    self._cur_parents.extend(emitter_parents)
                else:
                    self._cur_parents.extend([None] * len(emitter_sols))
            self._num_emitted[i] = len(emitter_sols)
    ...
    def tell(self, objective, measures, **fields):
        ...
        # Add the solutions to the lineage table.
        old_lineage_table = self._lineage_table

        solutions = data["solution"]
        measures = data["measures"]
        parents = self._cur_parents
        ...
        for i, sol in enumerate(solutions):
            ...
            elite = self.archive.retrieve_single(measures[i])[1]['solution']
            if any(sol != elite):
                continue

            # Add the solution to the lineage table.
            cell = self.archive.index_of_single(measures[i])
            if parents[i] is None:
                self._lineage_table[cell] = [cell]
            else:
                new_lineage = old_lineage_table[parents[i]].copy()
                new_lineage.append(cell)
                self._lineage_table[cell] = new_lineage

    def get_lineage_table(self):
