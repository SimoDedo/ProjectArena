...
if __name__ == "__main__":
    ...
    # Create random ABGenome
        ...
        print(np.std(graph.closeness(vertices=rooms, weights=graph.es['weight'])))
    ...
    print(len(cycles_one_room))
    print(np.mean(cor_length))
    print(np.std(cor_length))
    print(len(cycles_two_rooms))
    print(np.mean(ctr_length))
    print(np.std(ctr_length))

    layout = ig.Layout(coords=graph.vs['coords'], dim=2)
    fig, ax = plt.subplots()
    ig.plot(graph, vertex_size=25, layout=layout, vertex_label_color="white", edge_label=[str(i) for i in range(len(graph.es))],  target=ax)
    #plt.show()