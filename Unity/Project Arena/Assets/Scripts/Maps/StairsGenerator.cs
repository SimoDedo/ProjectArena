...
namespace Maps
{
    ...
    public class StairsGenerator : CoreComponent
    {
        ...
        public void GenerateStairs(List<char[,]> ms, MapGenerator.MapGenerator mg)
        {
            SetMapGenerationVariables(mg);
            maps = ms;

            for (var currentLevel = 0; currentLevel < maps.Count; currentLevel++)
                if (currentLevel > 0)
                    GenerateLevelStairs(GetPossibleStairs(currentLevel), currentLevel);
        }

        // Places stairs connecting adjacent levels of the map.
        public void GenerateStairs(List<char[,]> ms, List<bool> hasPlacedStairs, MapGenerator.MapGenerator mg,
            bool validateOnly)
        {
            SetMapGenerationVariables(mg);
            maps = ms;
            ...
            for (var currentLevel = 0; currentLevel < maps.Count; currentLevel++)
                if (currentLevel > 0)
                {
                    ...
                    else if (!validateOnly) GenerateLevelStairs(GetPossibleStairs(currentLevel), currentLevel);
                }
        }

        // Sets all the informations about the map generation.
        private void SetMapGenerationVariables(MapGenerator.MapGenerator mg)
        {
        ...
        private void GenerateLevelStairs(List<Stair> stairList, int currentLevel)
        {
            if (stairList.Count > 2)
            {
                ...
                {
                    var currentStair = stairList[mapGeneratorScript.GetRandomInteger(0,
                        stairList.Count)];
        ...
        {
            var stairList = new List<Stair>();

            // Detect and valdiate the stairs.
            for (var x = 0; x < maps[currentLevel].GetLength(0); x++)
            for (var y = 0; y < maps[currentLevel].GetLength(1); y++)
