...
namespace Managers.Mode
{
    ...
    public class TargetRushGameManager : GameManager
    {
        ...
        private IEnumerator SpawnWave()
        {
            ...
            foreach (var target in waveList[currentWave - 1].targetList) targetsCount += target.count;
            targetRushGameUIManagerScript.SetTargets(targetsCount);
            ...
            // Spawn the targets.
            foreach (var target in waveList[currentWave - 1].targetList
                ) // Debug.Log("Spawning " + target.count + " " + target.prefab.name + " in wave " 
                // + currentWave + ".");
                for (var i = 0; i < target.count; i++)
                {
                    var newTarget = Instantiate(target.prefab);
