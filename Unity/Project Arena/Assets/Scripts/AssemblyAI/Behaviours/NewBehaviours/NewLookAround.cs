...
{
    [Serializable]
    public class NewLookAround : Action
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (Vector3.Dot(realForward, previousVelocity) < 0.8)
            {
                angleX = 0;
            }
            else if (mustFindAngle || Input.GetKeyDown(KeyCode.A))
            {
        ...
        private const float DISTANCE_THRESHOLD = 15f;
        ...
        private static readonly ReadOnlyCollection<AngleScore> angleScores = new ReadOnlyCollection<AngleScore>(new[]
        {
            ...
            new AngleScore {angle = +135, score = 10, minimumLevel = AIEntity.CuriosityLevel.High},
            new AngleScore {angle = +180, score = 5, minimumLevel = AIEntity.CuriosityLevel.High},
        });
    }
}