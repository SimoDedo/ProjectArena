...
namespace Polimi.Gamecollective.Connectivity
{
	...
	/// parameters to specify the full url to the server (including the directory), the encryption key,
	///  the encryption initialization vector (IV) and the salt. Only one send/retrive request
	/// can be submitted at a time. Check IsRequestPending to know if you can submit a new one, and 
	/// IsResultReady for knowing when your data is ready, in case of retrieval (check them for example
	/// in the Update() method of another component). Retrieved data is in the Result field once it arrives.
	/// The Result field is overwritten with the empty string when a new retrieval request is sent.
	/// </summary>
